...
while ($equipcategory->nextRow()) {
		...
		while ($so_equip->nextRow()) {
			...
			$soequippengembalian->selectByParamsMonitoring(array("A.SO_EQUIP_ID"=>$so_equip->getField("SO_EQUIP_ID")));
			$soequippengembalian->firstRow();

			$inConditon =$so_equip->getField("IN_CONDITION");
			$inConditon =$inConditon?$inConditon:$soequippengembalian->getField("IN_CONDITION");
			$reqRemark  =$so_equip->getField('REMARK');
			$reqRemark = $soequippengembalian->getField("REMARK")?$soequippengembalian->getField("REMARK"):$reqRemark;

			$aData[$nomer]["EQUIP_NAME"]=$so_equip->getField("EQUIP_NAME");
			$aData[$nomer]["QTY"]=$so_equip->getField("QTY");
			$aData[$nomer]["EQUIP_ITEM"]=$so_equip->getField("EQUIP_ITEM");
			$aData[$nomer]["OUT_CONDITION"]=$so_equip->getField("OUT_CONDITION");
			$aData[$nomer]["IN_CONDITION"]=$inConditon;
			$aData[$nomer]["REMARK"]=$reqRemark;
			$aData[$nomer]["FLAG"]=$soequippengembalian->getField("FLAG");
			$nomer++;
		}
...
<table style="width: 100%;font-size: 13px;font-family: calibri;border-collapse: 1px solid black; margin-top: -10px" border="1">
	...
	<tr>
		...
		<!-- <td style="padding: 5px" align="center"><input type="checkbox" name="" <?=$chacked?>> </td> -->
	</tr>
	<?		
...
<span style="font-size: 9px;font-family: calibri;"><em>(*) Notes : G = Good, B= Broken, M = Miss  </em> </span>
<br>
<br>
<table style="width: 100%;border-collapse: 1px solid black;font-size: 13px;font-family:calibri;text-align: center;" border="1">
	...
	<tr >
		...
		<td><b>Note <b></td>
		<td colspan="2"><b>Remark on item conditions<b></td>
