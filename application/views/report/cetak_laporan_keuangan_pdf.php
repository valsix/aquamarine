...
$pdf->Cell($panjang, 10, 'LAPORAN KEUANGAN', 0, 0, 'C');
$pdf->Ln(10);
...
$form->selectByParamsDokumentCetakPdf(array(), -1, -1, $statement . $statement_privacy);
// echo $form->query;
// exit;
$pdf->SetFont('Arial', '', 8);
...
$no = 1;
while ($form->nextRow()) {
    // $date1 =  $form->getField('DATE1');
    // $date2 =  $form->getField('DATE2');
    // $date_val =
    //     $pdf->Row(array(
    //         $no,
    //         '' . $form->getField($aColumns[1]),
    //         '' . $form->getField($aColumns[2]),
    //         '' . $form->getField($aColumns[3]),
    //         '' . $form->getField($aColumns[4]),
    //         '' . getTglBlnTahun($date1) . ' - ' . getTglBlnTahun($date2),
    //         '' . $form->getField($aColumns[6]),
    ...
    $pdf->Row(array(
        ...
        '' . $form->getField($aColumns[3]),
        '' . $form->getField($aColumns[4]),
        '' . $form->getField($aColumns[5]),
        '' . $form->getField($aColumns[6])
    ));

    $pdf->MultiCell($panjang - 2.2, 5, 'EXP : ' . $form->getField('EXP'), 1);

    // $pdf->MultiCell($panjang - 2.2, 5, 'COMPANY NAME : ' . "\t" . $form->getField($aColumns[7]), 1, 'J', 0, 10);
    // $pdf->MultiCell($panjang - 2.2, 5, 'VESSEL NAME : ' . $form->getField('VESSEL_NAME'), 1);
    // $pdf->MultiCell($panjang - 2.2, 5, 'PPN : ' . currencyToPage2($form->getField('PPN')), 1);
    // $pdf->MultiCell($panjang - 2.2, 5, 'STATUS : ' . currencyToPage2($form->getField('STATUS')), 1);
    // $pdf->MultiCell($panjang - 2.2, 5, 'TOTAL AMOUNT: ' . currencyToPage2($form->getField('TOTAL_AMOUNT')), 1);
    // $pdf->MultiCell($panjang - 2.2, 5, 'INVOICE DATE : ' . $form->getField('INVOICE_DATE'), 1);
    $no++;
}
