...
$reqEcommerceId = $this->input->get("reqEcommerceId");

if($reqEcommerceId != ""){
	$ecommerce->selectByParamsMonitoring(array("ECOMMERCE_ID" => $reqEcommerceId));
	$ecommerce->firstRow();

	$reqName1 = $ecommerce->getField("EMAIL");
	$reqName2 = $ecommerce->getField("CC");
	$reqName3 = $ecommerce->getField("BCC");
	$reqSubject = $ecommerce->getField("SUBJECT");
	$reqDescription = $ecommerce->getField("BODY");
	$reqIdLampiran = $ecommerce->getField("ATTACHMENT");
...
<div class="col-md-12">
 ...
 -->
					...
					<tr>
						...
						<td><input type="text" class="form-control" name="reqSubject" value="<?=$reqSubject?>" />
...
<script type="text/javascript">
	...
	function load_history() {
		openAdd('app/loadUrl/app/template_load_ecommerce');
	}
	$(document).ready(function() {
		var reqName1 = "<?=$reqName1?>";
		var reqName2 = "<?=$reqName2?>";
		var reqName3 = "<?=$reqName3?>";

		reqName1 = reqName1.split(",");
		reqName2 = reqName2.split(",");
		reqName3 = reqName3.split(",");

		setTimeout(function() {
			for (var i = 0; i < reqName1.length; i++) {
				$('#reqName1').tagsinput('add',reqName1[i]);
			}
			for (var i = 0; i < reqName2.length; i++) {
				$('#reqName2').tagsinput('add',reqName2[i]);
			}
			for (var i = 0; i < reqName3.length; i++) {
				$('#reqName3').tagsinput('add',reqName3[i]);
			}
		}, 1000);
		
		if("<?=$reqIdLampiran?>" != ""){
			$.get("web/document_attacment_json/add_path_lampiran?reqId=<?=$reqIdLampiran?>", function(data) {
...
<script type="text/javascript">
	function submitForm() {
		...
		$('#ff').form('submit', {
			url: 'web/customer_json/sending_mail',
			onSubmit: function() {
			...
			success: function(data) {
				...
				// alert(data);
				$.messager.alertLink('Info', data, 'info', "app/index/e_commerce?");
			}
		});
	}
</script>
