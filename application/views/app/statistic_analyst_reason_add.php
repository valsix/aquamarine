...
if ($reqId == "") {
    ...
    $statement =" AND TO_CHAR( A.DATE_OF_SERVICE,'YYYY')='".$reqIds[1]."' AND A.MASTER_REASON_ID IS NOT NULL";
    ...
    while ( $project_hpp->nextRow()) {
        array_push($arrData, $project_hpp->getField('MASTER_REASON_ID'));
    }
    // print_r($arrData);exit;
    $arraUnix = array_unique($arrData);

    $reqMasterReasonId = implode(',', $arraUnix);
    if(!empty($reqMasterReasonId)){
        $statement = " AND A.MASTER_REASON_ID IN(".$reqMasterReasonId.")";
    }else{
         $statement = " AND A.MASTER_REASON_ID ='-2'";
    }
                    ...
                    <table class="table table-bordered" style="width: 100%">
                        ...
                        <tbody id="tambahAksi">
                                ...
                                while ( $master_lokasi->nextRow()) {
                                    $project_hpp =  new Offer();
                                    $stotal = $project_hpp->getCountByParams(array("A.MASTER_REASON_ID"=>$master_lokasi->getField("MASTER_REASON_ID")));
                                    $total = $total+$stotal;
                                   ?>
                                   <tr>
       ...
       $(document).ready(function() {
           setTimeout(function () {
             ...
             // $('.grap').each(function(){
                ...
                function drawChart() {
                    $.getJSON("web/statistic_analyst_json/getGoogleChartReason?reqId="+grafikId+"&reqTahun=<?=$reqTahun?>", function(dataJSON) {
