...
if ($reqId == "") {
    ...
    $reqMode = "ubah";
    $statement = " AND A.REPORT_ID = " . $reqId;
    $jenis_kualifikasi->selectByParamsReport(array(), -1, -1, $statement);

    $jenis_kualifikasi->firstRow();
    $reqId          = $jenis_kualifikasi->getField("REPORT_ID");
    $reqJenis       = $jenis_kualifikasi->getField("REPORT");
            ...
            <div>
                <form id="ff" class="easyui-form form-horizontal" method="post" novalidate enctype="multipart/form-data">
                    ...
                    <div class="page-header">
                        <h3><i class="fa fa-file-text fa-lg"></i> Master Qualification</h3>
                    </div>
                ...
                <table style="width: 100%" id="tablei" class="table table-striped table-hover dt-responsive" style="background: white">
                    ...
                    <tbody id="tambahAttacment">
                        <?
                        $jenis_kualifikasi = new Report();
                        $jenis_kualifikasi->selectByParamsReport(array());
                        while ($jenis_kualifikasi->nextRow()) {

                            $reqIds             = $jenis_kualifikasi->getField("REPORT_ID");
                            $reqJenis           = $jenis_kualifikasi->getField("REPORT");
                            $reqDescription     = $jenis_kualifikasi->getField("DESCRIPTION");
                        ?>
                            <tr>
                                <td> <a onclick="tambahPenyebab(<?= $reqIds ?>)" id="btnPenyebab" class="btn btn-info"><i class="fa fa-pencil"></i></a>
                                 <a onclick="hapus(<?= $reqIds ?>)" id="btnPenyebab" class="btn btn-danger"><i class="fa fa-trash"></i></a></td>
                                <td><?= $reqJenis ?> </td>
...
<script type="text/javascript">
    ...
    function hapus(id){
        deleteData('web/jenis_kualifikasi_json/delete',id);        
    }
