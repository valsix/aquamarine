...
$reqTenderId = $this->input->get("reqTenderId");
$reqType = $this->input->get("reqType");
$arrMenu = array('dokument_adminitrasi','dokument_teknis','dokument_komersial');
$pageHead = $reqType;
if($reqType=='dokument_adminitrasi'){
    $pageHead ='Dokumen Administrasi';
}else if($reqType=='dokument_teknis'){
     $pageHead ='Dokumen Teknis';
}else if($reqType=='dokument_komersial'){
     $pageHead ='Dokumen Komersial';
}
if ($reqId == "") {
    $reqMode = "insert";
} else {
    $reqMode = "ubah";
    $statement = " AND A.TENDER_TYPE_UPLOAD_ID = " . $reqId;
    $tender_type_upload->selectByParamsMonitoring(array(), -1, -1, $statement);

    $tender_type_upload->firstRow();
    $reqId = $tender_type_upload->getField("TENDER_TYPE_UPLOAD_ID");
    $reqName = $tender_type_upload->getField("NAME");
...
<div class="col-md-12">
    ...
    <div class="judul-halaman">Master Type <?=str_replace("_", ' ', $pageHead)?>  </div>
            ...
            <div>
                <form id="ff" class="easyui-form form-horizontal" method="post" novalidate enctype="multipart/form-data">
                    ...
                    <div class="page-header">
                        <h3><i class="fa fa-file-text fa-lg"></i> Type <?=str_replace("_", ' ', $pageHead)?> </h3>
                    </div>
                    ...
                    <input type="hidden" name="reqTenderId" value="<?= $reqTenderId ?>" />
                ...
                <table style="width: 100%" id="tablei" class="table table-striped table-hover dt-responsive" style="background: white">
                    ...
                    <tbody id="tambahAttacment">
                        <?
                        $tender_type_upload = new TenderTypeUpload();
                        $tender_type_upload->selectByParamsMonitoring(array("TYPE" => $reqType, "TENDER_ID" => $reqTenderId));
                        while ($tender_type_upload->nextRow()) {

                            $reqIds = $tender_type_upload->getField("TENDER_TYPE_UPLOAD_ID");
...
<script type="text/javascript">
    function submitForm() {
        $('#ff').form('submit', {
            url: 'web/type_tender_upload_json/add',
            onSubmit: function() {
            ...
            success: function(data) {
                ...
                //alert(data);
                $.messager.alertLink('Info', data, 'info', "app/loadUrl/app/tempalate_master_type_tender?reqType=<?=$reqType?>&reqTenderId=<?=$reqTenderId?>");
            }
        });
    }
    ...
    function hapus(id){
        deleteData('web/type_tender_upload_json/delete',id);
    }

    function tambahPenyebab(id) {
        window.location.href = "app/loadUrl/app/tempalate_master_type_tender?reqType=<?=$reqType?>&reqTenderId=<?=$reqTenderId?>&reqId=" + id;
