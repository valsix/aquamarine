...
if ($reqId == "") {
    ...
    $reqMode = "ubah";
    $statement = " AND A.ISSUE_PO_TYPE_ID = " . $reqId;
    $issue_po_type->selectByParamsMonitoring(array(), -1, -1, $statement);

    $issue_po_type->firstRow();
    $reqId = $issue_po_type->getField("ISSUE_PO_TYPE_ID");
    $reqName = $issue_po_type->getField("NAME");
    $reqDescription = $issue_po_type->getField("DESCRIPTION");
}
...
<div class="col-md-12">
    ...
    <div class="judul-halaman">Master Type PO </div>
            ...
            <div>
                <form id="ff" class="easyui-form form-horizontal" method="post" novalidate enctype="multipart/form-data">
                    ...
                    <div class="page-header">
                        <h3><i class="fa fa-file-text fa-lg"></i> Type PO </h3>
                    </div>
                ...
                <table style="width: 100%" id="tablei" class="table table-striped table-hover dt-responsive" style="background: white">
                    ...
                    <tbody id="tambahAttacment">
                        <?
                        $issue_po_type = new IssuePoType();
                        $issue_po_type->selectByParamsMonitoring();
                        while ($issue_po_type->nextRow()) {

                            $reqIds = $issue_po_type->getField("ISSUE_PO_TYPE_ID");
                            $reqJenis           = $issue_po_type->getField("NAME");
                            $reqDescription     = $issue_po_type->getField("DESCRIPTION");
                        ?>
                            <tr>
...
<script type="text/javascript">
    function submitForm() {
        $('#ff').form('submit', {
            url: 'web/issue_po_type_json/add',
            onSubmit: function() {
            ...
            success: function(data) {
                ...
                //alert(data);
                $.messager.alertLink('Info', data, 'info', "app/loadUrl/app/tempalate_master_type_po");
            }
        });
    }
    ...
    function hapus(id){
        deleteData('web/issue_po_type_json/delete',id);
    }

    function tambahPenyebab(id) {
        window.location.href = "app/loadUrl/app/tempalate_master_type_po?reqId=" + id;
