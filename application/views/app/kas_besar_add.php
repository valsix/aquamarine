...
$aColumns = array("KAS_BESAR_DETAIL_ID", "NO", "TANGGAL", "KETERANGAN", "KATEGORI", "KREDIT ( RP. )", "DEBET ( RP. )", "BALENCE ( RP. )","KREDIT ( USD )","DEBET (USD )","BALENCE (USD)", "AKSI");
...
if ($reqId == "") {
    ...
    // echo $kas_besar->query;exit;
    $kas_besar->firstRow();

    $reqTanggal = $kas_besar->getField("TANGGAL");
    $reqDeskripsi = $kas_besar->getField("DESKRIPSI");
    $reqBankId = $kas_besar->getField("BANK_ID");

    $is_approve = $kas_besar->getApproval(array("A.KAS_BESAR_ID" => $reqId));
}
...
<script type="text/javascript" language="javascript" class="init">
    ...
    $(document).ready(function() {
        ...
        oTable = $('#example').dataTable({
            ...
            "sAjaxSource": "web/kas_besar_detail_json/json?reqId="+reqIds,
            ...
            "footerCallback": function(row, data, start, end, display) {
                ...
                var totalKreditUSD = api.column(8).data().reduce(function(a, b) {
                    return getFloatVal(a) + getFloatVal(b);
                }, 0);
 // console.log(totalKreditUSD);

                var totalDebetUSD = api.column(9).data().reduce(function(a, b) {
                    return getFloatVal(a) + getFloatVal(b);
                }, 0);

                var totalBalanceUSD = totalKreditUSD-totalDebetUSD  ;

                var totalKredit = totalKreditIDR;
                var totalDebet = totalDebetIDR;
                var totalBalance = totalBalanceIDR;

                if(totalKredit == 0 && totalDebet == 0 && totalBalance == 0) {
        ...
        $('#btnEdit').on('click', function() {
            ...
            $("#reqKasBesarDetailId").val(elements[0]);
...
<div class="col-md-12">
    ...
    <div class="judul-halaman"> <a href="app/index/kas_besar"> Kas Besar</a> &rsaquo; Form Kas Besar
                ...
                <form id="ff" class="easyui-form form-horizontal" method="post" novalidate enctype="multipart/form-data">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <div class="col-md-11">
                                     ...
                                     <input class="easyui-combobox form-control " style="width:100%" id="reqBankId" name="reqBankId" data-options="width:'372',editable:true, valueField:'id',textField:'text',url:'combo_json/ComboBank'" value="<?=$reqBankId?>" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <div class="col-md-11">
                                    <input type="hidden" name="reqKasBesarDetailId" id="reqKasBesarDetailId">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <div class="col-md-11">
                                    ...
                                    <input type="text" id="reqKredit" class="easyui-validatebox form-control" name="reqKredit" value="" style="margin-top: 6px; width:70%" onchange="numberWithCommas('reqKredit<?= $ids_id ?>')" onkeyup="numberWithCommas('reqKredit<?= $ids_id ?>')" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <div class="col-md-11">
                                    ...
                                    <input type="text" id="reqDebet"  class="easyui-validatebox form-control" name="reqDebet" value="" style="margin-top: 6px; width:70%" onchange="numberWithCommas('reqDebet<?= $ids_id ?>')" onkeyup="numberWithCommas('reqDebet<?= $ids_id ?>')" />
                                </div>
                            </div>
                        </div>
    ...
    <script>
        function submitForm() {
            $('#ff').form('submit', {
                url: 'web/kas_besar_detail_json/add',
                onSubmit: function() {
                ...
                success: function(data) {
                    ...
                    if (datas[1] != '') {
                        ...
                        show_toast('info', 'Information', 'Header success added'+datas[1]);
                          $.messager.alertLink('Info', datas[0], 'info', "app/index/kas_besar_add/?reqId="+datas[1]);
    ...
    <script type="text/javascript">
        ...
        function preview_pdf(){
                openAdd('app/loadUrl/report/cetak_document_kas_besar_pdf?reqId=<?=$reqId?>');
        }
        ...
        function deleting(id) {
            ...
            $.get("web/kas_besar_detail_json/delete?reqId=" + elements[0], function(data) {
                oTable.api().ajax.reload(null,false);
        ...
        function approve(){
            $.get("web/kas_besar_json/approve?reqId=<?=$reqId?>", function(data) {
                var datas = data.split('-');
                show_toast('info', 'Information', 'Header success added'+datas[1]);
                $.messager.alertLink('Info', datas[0], 'info', "app/index/kas_besar_add/?reqId="+datas[1]);
            });
        ...
        function batal_approve(){
            $.get("web/kas_besar_json/approve_cancel?reqId=<?=$reqId?>", function(data) {
                var datas = data.split('-');
                show_toast('info', 'Information', 'Header success added'+datas[1]);
                $.messager.alertLink('Info', datas[0], 'info', "app/index/kas_besar_add/?reqId="+datas[1]);
            });
        ...
        function editing(id) {
           ...
           var debet_idr =elements[6];
            var kredit_idr =elements[5];
            var debet_usd  =elements[9];
            var kredit_usd =elements[8];
             console.log(kredit_usd);
            if( debet_idr != '0,00' ||  kredit_idr != '0,00'){
    ...
    <script type="text/javascript">
        function reload_detail() {
            ...
            $.get("web/cash_besar_detail_json/getSaldoAkhir?reqId=<?= $reqId ?>" , function(data) {

                $("#reqSaldo").html(data);

            });

             $.get("web/cash_besar_detail_json/getSaldoAkhirUSD?reqId=<?= $reqId ?>" , function(data) {
