...
if ($reqId == "") {
    ...
    $reqMode = "ubah";
    $statement = " AND A.TYPE_CONTRACT_ID = " . $reqId;
    $type_contract->selectByParamsMonitoring(array(), -1, -1, $statement);

    $type_contract->firstRow();
    $reqId = $type_contract->getField("TYPE_CONTRACT_ID");
    $reqName = $type_contract->getField("NAME");
    $reqDescription = $type_contract->getField("DESCRIPTION");
}
                ...
                <table style="width: 100%" id="tablei" class="table table-striped table-hover dt-responsive" style="background: white">
                    ...
                    <tbody id="tambahAttacment">
                        ...
                        while ($type_contract->nextRow()) {
                            ...
                            $reqIds = $type_contract->getField("TYPE_CONTRACT_ID");
                            $reqJenis           = $type_contract->getField("NAME");
                            $reqDescription     = $type_contract->getField("DESCRIPTION");
                        ?>
                            <tr>
