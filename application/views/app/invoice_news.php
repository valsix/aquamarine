...
$reqStatus  = $_SESSION[$pg."reqStatus"];
...
if(!empty($reqLokasi)){
      $statament  .=" AND EXISTS( SELECT 1 FROM PROJECT_HPP_NEW DD LEFT JOIN 
              MASTER_PROJECT CC ON   CC.MASTER_PROJECT_ID::VARCHAR = DD.CODE WHERE
              DD.PROJECT_HPP_NEW_ID = A.HPP_PROJECT_ID AND UPPER(DD.LOKASI) LIKE '%".strtoupper($reqLokasi)."%')";
}

if(!empty($reqStatus)){
      $statament  .="  AND UPPER(A.STATUS_INVOICE) LIKE '%".strtoupper($reqStatus)."%' ";
}

if(!empty($reqBulan) && !empty($reqTahun)){
      $statament  .="  AND TO_CHAR(A.INVOICE_DATE,'MMYYYY')='".$reqBulan.$reqTahun."'";
}

if(!empty($reqGlobal)){
     $statament  .="  AND ( 
     UPPER(A.nomer) LIKE '%".strtoupper($reqGlobal)."%' 
     OR UPPER(A.po_nomer      ) LIKE '%".strtoupper($reqGlobal)."%' 
     OR UPPER(A.deskripsi_payment  ) LIKE '%".strtoupper($reqGlobal)."%' 
     OR UPPER(A.adv_payment  ) LIKE '%".strtoupper($reqGlobal)."%' 
     OR UPPER(A.tax_invoice_nominal  ) LIKE '%".strtoupper($reqGlobal)."%' 
     OR UPPER(A.ro_number  ) LIKE '%".strtoupper($reqGlobal)."%' 
...
$offer = new InvoiceNew();

$total_row = $offer->getCountByParamsMonitoring(array(),$statament);

$serviceorder = new InvoiceNew();
$serviceorder->selectByParamsMonitoring(array(),-1,-1,$statament,' ORDER BY A.INVOICE_NEW_ID DESC');
...
<div class="col-md-12">
    ...
    <div class="judul-halaman"> Invoice Project</div>
                            ...
                            <tr>
                               ...
                               <tr>
                                ...
                                <td >   <input class="easyui-combobox form-control" placeholder="Date" style="width:190px" id="reqBulan" name="reqBulan" data-options="editable:false,height: 34, valueField:'id',textField:'text',url:'combo_json/ComboBulan'" value="<?= $reqBulan ?>" /> / <input type="text" name="reqTahun" class="easyui-textbox form-control" id="reqTahun" value="<?= $reqTahun ?>" style="width: 10%" maxlength="4" onkeypress='validate(event)'  Placeholder='Tahun'> </td>
                                  <td>Status</td>
                                <td>  <input class="easyui-combobox form-control" style="width:100%" name="reqStatus"  id="reqStatus" data-options="width:'250',editable:false, valueField:'id',textField:'text',url:'combo_json/comboStatus3?reqMode=ALL'" value="<?= $reqStatus ?>" /> </td>
                                <td> </td>
                            </tr>
        ...
        <table id="example2" class="table table-striped table-hover dt-responsive" cellspacing="0" width="100%" >
            ...
            <tbody>
                ...
                foreach ($arrData as  $value) {
                  ...
                  $class='';
                  if($value['status_invoice']=='' || $value['status_invoice']=='Pending'){
                     $class ="class='yellowClass' ";
                  }else if($value['status_invoice']=='Belum Lunas'){
                     $class ="class='redClass' ";
                  }
                ?>
                <tr >
                  <td <?= $class?>><?=$totalI;$totalI--;?>  </td>
                  <td <?= $class?>>  <?=$value['nomer']?> </td>
                  <td <?= $class?>> <?=$value['po_date']?> </td>
                  <td <?= $class?>> <?=$value['code']?> </td>
                  <td <?= $class?>>  <?=$value['company_name']?> </td>
                  <td <?= $class?>> <?=$value['nama_code_project']?> </td>
                  <td <?= $class?>> <?=$value['location']?> </td>
                  <td <?= $class?>>  <?=$value['invoice_day']?> </td>
                  <td <?= $class?>> <?=$value['status_invoice']?> </td>
                  <td <?= $class?>> <?=currencyToPage2($totalAmount)?> </td>
                    
                        
                           <td <?= $class?>>  <a href="javascript:void(0)" class="btn btn-warning" onclick="klikRubah(<?=$value['invoice_new_id']?>)"><i class="fa fa-fw fa-pencil"></i></a>
                            <?
                            if(empty($value['dari'])){
                              ?>

                             <a href="javascript:void(0)" class="btn btn-danger" onclick="delData(<?=$value['invoice_new_id']?>)"><i class="fa fa-fw fa-trash"></i></a> 
                          <?
                          }
                          ?>
                  </td>
...
<script type="text/javascript">
    ...
    order: [[0, 'desc']],"bStateSave": true,
         ...
         $('#btnAdd').on('click', function() {
            window.location.href='app/index/invoice_new_add';
        });
       });
function klikRubah(id){
        window.location.href='app/index/invoice_new_add?reqId='+id;
     }
     function delData(id){
        deleteData('web/invoice_new_json/delete',id);
     }
</script>
<!------>