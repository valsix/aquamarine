...
***************************************************************************************************** */
  ...
  ***/
  ...
  class IssuePoNonProjectDetail   extends Entity{
    ...
    * Class constructor.
    **/
    function IssuePoNonProjectDetail()
	{
      $this->Entity();
    }
    ...
    function insert()
    {
    	$this->setField("ISSUE_PO_DETAIL_ID", $this->getNextId("ISSUE_PO_DETAIL_ID","ISSUE_PO_NON_PROJECT_DETAIL")); 

    	$str = "INSERT INTO ISSUE_PO_NON_PROJECT_DETAIL ( ISSUE_PO_DETAIL_ID, KETERANGAN, QTY, SATUAN, AMOUNT, TOTAL,ISSUE_PO_ID,TERM,CURENCY, CREATED_BY,CREATED_DATE,STATUS_BAYAR)VALUES (
	...
	{
		$str = "
		UPDATE ISSUE_PO_NON_PROJECT_DETAIL
		SET    
	...
	{
		$str = "DELETE FROM ISSUE_PO_NON_PROJECT_DETAIL
		WHERE ISSUE_PO_DETAIL_ID= '".$this->getField("ISSUE_PO_DETAIL_ID")."'"; 
		$this->query = $str;
		  // echo $str;exit();
		return $this->execQuery($str);
	...
	{
		$str = "
		SELECT A.ISSUE_PO_DETAIL_ID,A.KETERANGAN,A.QTY,A.SATUAN,A.AMOUNT,A.TOTAL,A.CREATED_BY,A.CREATED_DATE,A.UPDATED_BY,A.UPDATED_DATE,A.CURENCY,A.TERM,A.STATUS_BAYAR,
		CASE 
		...
		FROM ISSUE_PO_NON_PROJECT_DETAIL A
		WHERE 1=1 ";
	...
	{
		$str = "SELECT COUNT(1) AS ROWCOUNT FROM ISSUE_PO_NON_PROJECT_DETAIL A WHERE 1=1 ".$statement;
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key = 	'$val' ";
		}
