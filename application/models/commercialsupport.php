...
class CommercialSupport    extends Entity
{
    ...
    function insert()
    {
        $this->setField("ID", $this->getNextId("ID","COMMERCIAL_SUPPORT")); 

        $str = "INSERT INTO COMMERCIAL_SUPPORT ( ID, PARENT_ID, NAMA, KET, CREATED_BY, CREATED_DATE)VALUES (
        '".$this->getField("ID")."',
    ...
    {
        $str = "DELETE FROM COMMERCIAL_SUPPORT
        WHERE ID= '".$this->getField("ID")."'"; 
        $this->query = $str;
          // echo $str;exit();
        return $this->execQuery($str);
    ...
    {
        $str = "SELECT COUNT(1) AS ROWCOUNT FROM COMMERCIAL_SUPPORT A WHERE 1=1 ".$statement;
        while(list($key,$val)=each($paramsArray))
        {
            $str .= " AND $key =    '$val' ";
        }
