...
***************************************************************************************************** */
  ...
  ***/
  ...
  class IssuePoNonProject  extends Entity{
    ...
    **/
    ...
    function insert()
    {
    	$this->setField("ISSUE_PO_ID", $this->getNextId("ISSUE_PO_ID","ISSUE_PO_NON_PROJECT")); 

    	$str = "INSERT INTO ISSUE_PO_NON_PROJECT ( ISSUE_PO_ID, NOMER_PO, PO_DATE, DOC_LAMPIRAN, REFERENSI, PATH_LAMPIRAN,        
    			...
    			TERMS_AND_CONDITION, NOTE, TYPE, ACKNOWLEDGED_BY, ACKNOWLEDGED_DEPT, APPROVED1_BY, STATUS,
	...
	{
		$str = "DELETE FROM ISSUE_PO_NON_PROJECT
		WHERE ISSUE_PO_ID= '".$this->getField("ISSUE_PO_ID")."'"; 
		$this->query = $str;
		  // echo $str;exit();
		return $this->execQuery($str);
	...
	function selectByParamsMonitoring($paramsArray=array(),$limit=-1,$from=-1, $statement="", $order="ORDER BY A.ISSUE_PO_ID ASC")
	{
		...
		SELECT A.ISSUE_PO_ID,A.NOMER_PO,A.PO_DATE,TO_CHAR(A.PO_DATE, 'DD/MM/YY') PO_DATE1,A.DOC_LAMPIRAN,A.REFERENSI,A.PATH_LAMPIRAN,A.FINANCE,A.COMPANY_ID,A.COMPANY_NAME,A.CONTACT,A.ADDRESS,A.EMAIL,A.TELP,A.FAX,A.HP,A.BUYER_ID,A.OTHER,A.PPN,A.PPN_PERCENT,A.CREATED_BY,A.CREATED_DATE,A.UPDATED_BY,A.UPDATED_DATE, NOTE, TYPE, ACKNOWLEDGED_BY, ACKNOWLEDGED_DEPT, APPROVED1_BY, APPROVED1_DEPT, APPROVED2_BY, APPROVED2_DEPT,
			A.PIC, A.DEPARTEMENT, A.TERMS_AND_CONDITION,A.TYPE_CUR,
			CASE
			(SELECT COUNT (1) FROM ISSUE_PO_NON_PROJECT_DETAIL X WHERE A.ISSUE_PO_ID = X.ISSUE_PO_ID AND X.STATUS_BAYAR = 2)
			WHEN 0 THEN ''
			ELSE 'red'
			END STATUS3,
			CASE
			...
			ELSE ''
			END STATUS_KET,
			(SELECT MAX(KETERANGAN) FROM ISSUE_PO_NON_PROJECT_DETAIL X WHERE A.ISSUE_PO_ID = X.ISSUE_PO_ID) DESCRIPTION,A.STATUS,
			(SELECT SUM(TOTAL) FROM ISSUE_PO_NON_PROJECT_DETAIL X WHERE A.ISSUE_PO_ID = X.ISSUE_PO_ID) TOTAL,B.NAMA CUR,B.INISIAL,B.FORMAT,C.NAMA STATUS_NAMA,A.TANGGAL_BAYAR,A.DISKON,A.DISKON_PERCENT,A.STATUS_BAYAR
		FROM ISSUE_PO_NON_PROJECT A
		LEFT JOIN MASTER_CURRENCY B ON CAST(B.MASTER_CURRENCY_ID AS VARCHAR) = A.TYPE_CUR
		LEFT JOIN MASTER_STATUS_BAYAR C ON CAST(C.MASTER_STATUS_BAYAR_ID AS VARCHAR) = A.STATUS
		WHERE 1=1 ";
	...
	{
		$str = "SELECT COUNT(1) AS ROWCOUNT FROM ISSUE_PO_NON_PROJECT A WHERE 1=1 ".$statement;
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key = 	'$val' ";
		}
