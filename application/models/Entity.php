  ...
  class Entity extends CI_Model{ 
    ...
    **/
	...
	public function getNextId($idName,$tableName,$filter=""){
		$str = "SELECT MAX($idName) nilai FROM $tableName ";
		if($filter != "")
			$str = $str . " WHERE $filter";		
			$query = $this->db->query($str);
			$row = $query->first_row();
	...
	function selectLimit($sql,$numrow=-1,$offset=-1){
		...
		if(!$rs){
   			...
   			$this->errorPosgresql();
			$this->errorMsg = "PESAN ERROR : ".$errMess;
	...
	function execQuery($sql){
			$reqHalaman = $_SESSION['HALAMAN_PATH'];
			$userId= $_SESSION['USERIDX'];
			$arrData = explode(' ', trim(strtoupper($sql)));
			$reqModul =array();
			for($i=0;$i<3;$i++){
				if(!empty($arrData[$i])){
					array_push($reqModul, $arrData[$i]);
				
				}
			}
			$reqNamaTable = $reqModul[1];
			$reqModul = $reqModul[0];
			
			// $reqModul = str_replace(' ', '',$reqModul);
			...
			if( $DELETE  || $UPDATE  || $INSERT ){
				$REQlOGID= $this->getNextId("LOG_BARU_ID", "LOG_BARU");
				

				  $data = array(
				  	'log_baru_id' => $REQlOGID,
				  	'halaman' => $reqHalaman,
				  	'modul' => $reqModul,
				  	'data_sql' => $sql,
				  	'tabel_log' => $reqNamaTable,
				  	'created_by' => $userId
				  	
				  );

				  $this->db->insert('log_baru', $data);
	...
	function errorPosgresql(){
		$errNo   = $this->db->_error_number();
		$errMess = $this->db->_error_message();

		if (strpos($errMess, 'invalid input syntax for integer') !== false) {
