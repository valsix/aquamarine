...
class master_tender_json extends CI_Controller
{
	...
	function json()
	{
		...
		$dokumen = new MasterTender();
		// var_dump($dokumen);
		// echo $reqKategori;exit;

		$aColumns		= array("MASTER_TENDER_ID", "NO_PROJECT", "KETERANGAN","COMPANY_NAME","AKSI",COMPANY_ID);
		$aColumnsAlias	= array("MASTER_TENDER_ID", "NO_PROJECT", "KETERANGAN","COMPANY_NAME","AKSI",COMPANY_ID);
		...
		if (isset($_GET['iSortCol_0'])) {
			...
			if (trim($sOrder) == "ORDER BY A.MASTER_TENDER_ID asc") {
				/*
				...
				*/
				$sOrder = " ORDER BY A.MASTER_TENDER_ID desc";
			}
		}
		...
		$statement = " AND (UPPER(A.NO_PROJECT) LIKE '%" . strtoupper($_GET['sSearch']) . "%')";
	...
	function delete(){
		...
		$bank = new MasterTender();
		$bank->setField('MASTER_TENDER_ID',$reqId);
		$bank->delete();
		echo 'Data barhasil di delete';
	}

	function combo_master(){
		$this->load->model("MasterTender");
		$i=0;
		$master_tender = new MasterTender();
		$master_tender->selectByParamsMonitoring(array());
		$arrData = array();
		while ( $master_tender->nextRow()) {
			$arrData[$i]['id']=$master_tender->getField("NO_PROJECT");
			$arrData[$i]['text']=$master_tender->getField("NO_PROJECT");
			$i++;
		}
	...
	function add()
	{
		...
		$bank->setField("MASTER_TENDER_ID", $reqId);
		$bank->setField("NO_PROJECT", $reqJenis);
		$bank->setField("KETERANGAN", $reqDescription);
		$bank->setField("COMPANY_ID", $reqCompanyId);
		$bank->setField("COMPANY_NAME", $reqCompanyName);
