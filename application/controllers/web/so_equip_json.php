...
class so_equip_json extends CI_Controller
{
	...
	function json()
    {
        ...
        $aColumns = array("SO_EQUIP_ID", "EQUIP_ID","EC_NAME","EQUIP_NAME","SERIAL_NUMBER","QTY","EQUIP_ITEM","OUT_CONDITION","IN_CONDITION","REMARK", "AKSI",'KEMBALI' );

        $aColumnsAlias = array("SO_EQUIP_ID", "EQUIP_ID","EC_NAME","EQUIP_NAME","SERIAL_NUMBER","QTY","EQUIP_ITEM","OUT_CONDITION","IN_CONDITION","REMARK", "AKSI",'KEMBALI' );
        /*
		 * Ordering
		 */
    ...
    function add(){
        ...
        $soequip = new SoEquip();
        $total_flag_1 = $soequip->getCountByParamsMonitoringEquips(array("A.EQUIP_ID"=>$reqEquipId,"A.FLAG"=>'1'));

        if(empty($reqSoEquipId)){

         if( empty($reqEquipId)  ){
                   echo 'Data Gagal di simpan Belum ada item terpilih ';
                exit;
             }


         if( $total_flag_1 > 0){
                   echo 'Data Gagal di simpan Item sedang keluar';
                exit;
             }
        }     
