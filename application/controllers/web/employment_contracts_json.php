...
class employment_contracts_json extends CI_Controller
{
    ...
    function json()
    {
        ...
        $aColumns = array(
            "DOCUMENT_ID","NO", "TYPE", "TAHUN", "NAME", "DESCRIPTION", "PATH", "LAST_REVISI"
        );
        ...
        if (isset($_GET['iSortCol_0'])) {
            ...
            if (trim($sOrder) == "ORDER BY TAHUN DESC") {
                /*
        ...
        $_SESSION[$this->input->get("pg")."reqType"] = $reqType;
        ...
        $allRecord = $employmentContracts->getCountByParams(array(), $statement_privacy . $statement);
        // echo $allRecord;exit;
        if ($_GET['sSearch'] == "")
            $allRecordFilter = $allRecord;
        else
            $allRecordFilter =  $employmentContracts->getCountByParams(array(), $statement_privacy . $statement);

        $employmentContracts->selectByParams(array(), $dsplyRange, $dsplyStart, $statement_privacy . $statement, $sOrder);
        // echo $employmentContracts->query;
        // exit;
        ...
        $output = array(
         ...
         $nomer=0;
        while ($employmentContracts->nextRow()) {
            $row = array();
            ...
            for ($i = 0; $i < count($aColumns); $i++) {
               if ($aColumns[$i] == "NO") {
                ...
                $row[] = $employmentContracts->getField($aColumns[$i]);
                }
            }
            $output['aaData'][] = $row;
             $nomer++;
    ...
    function add()
    {
        ...
        $employmentContracts->setField("DOCUMENT_ID", $reqId);
        $employmentContracts->setField("TYPE", $reqType);
        $employmentContracts->setField("NAME", $reqName);
        $employmentContracts->setField("DESCRIPTION", $reqDescription);
        $employmentContracts->setField("PATH", $reqPath);
        $employmentContracts->setField("LAST_REVISI", $reqLastRevisi);
        $employmentContracts->setField("TAHUN", trim($reqTahun));

        if ($reqMode == "insert") {
            $employmentContracts->insert();
            $reqId = $employmentContracts->id;
        } else {
