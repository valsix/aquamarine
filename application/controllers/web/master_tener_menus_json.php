...
class master_tener_menus_json   extends CI_Controller
{
	...
	function json()
	{
		...
		$dokumen = new MasterTenerMenus();
		// var_dump($dokumen);
		// echo $reqKategori;exit;

		$aColumns		= array("MASTER_TENDER_MENUS_ID","NAMA","ALIAS","COLOR","COLOR2","URUTAN","AKSI");
		$aColumnsAlias	= array("MASTER_TENDER_MENUS_ID","NAMA","ALIAS","COLOR","COLOR2","URUTAN","AKSI");
	...
	function delete(){
		...
		$bank = new MasterTenerMenus();
		$bank->setField('MASTER_TENDER_MENUS_ID',$reqId);
		$bank->delete();
	...
	function add()
	{
		...
		$reqColor2 	= $this->input->post("reqColor2");
		$reqUrutan 	= $this->input->post("reqUrutan");

		$master_tener_menus = new MasterTenerMenus();
		$master_tener_menus->selectByParamsMonitoring(array("CAST(A.URUTAN AS VARCHAR)"=>$reqUrutan));
		while ( $master_tener_menus->nextRow()) {
			$Urutan = $master_tener_menus->getField('URUTAN');
			if($Urutan == $reqUrutan){
				if($reqId ==$master_tener_menus->getField('MASTER_TENDER_MENUS_ID') ){

				}else{
					echo 'xxx-Nomer Urutan sudah di gunakan';exit;
				}
			}
			# code...
		}
		...
		$bank->setField("ALIAS", $reqDescription);
		$bank->setField("COLOR", $reqColor);
		$bank->setField("COLOR2", $reqColor2);
		$bank->setField("URUTAN", $reqUrutan);
		...
		echo 'OK-Data Berhasil di simpan';
	}
}
