...
class customer_json extends CI_Controller
{
    ...
    {   
        ...
        $aColumns = array(
            "COMPANY_ID", "", "CHECK","NO", "NAME", "CP1_NAME", "PHONE","CP1_TELP", "EMAIL", "ADDRESS", "FAX",
            "CP1_TELP","SUPPORT"
        );
        $aColumnsAlias = array(
            "COMPANY_ID", "", "CHECK", "NAME", "CP1_NAME", "PHONE","CP1_TELP", "EMAIL", "ADDRESS", "FAX",
            "CP1_TELP", "SUPPORT"
        );
    ...
    function tambah_data_part($reqId){
        $this->load->model("SupplierPart");

       
        $reqQty = $this->input->post('reqQtyPart');
        $reqHarga = $this->input->post('reqHargaPart');
        $reqBarangNama = $this->input->post('reqBarangNamaPart');
        $reqSupplierBarangId = $this->input->post('reqSupplierBarangIdPart');
        $reqSerialPart= $this->input->post('reqSerialPart');
         $reqCurrencyPart= $this->input->post('reqCurrencyPart');
        for($i=0;$i<count($reqSupplierBarangId);$i++){
            $reqSubBarngId = $reqSupplierBarangId[$i];
            $supplierbarang = new SupplierPart();
            $supplierbarang->setField("SUPPLIER_PART_ID",$reqSubBarngId);
            $supplierbarang->setField("NAMA",$reqBarangNama[$i]);
             $supplierbarang->setField("SERIAL_NUMBER",$reqSerialPart[$i]);
            $supplierbarang->setField("QTY",ifZero2(dotToNo($reqQty[$i])));
            $supplierbarang->setField("HARGA",ifZero2(dotToNo($reqHarga[$i])));
          $supplierbarang->setField("CURRENCY",$reqCurrencyPart[$i]);
    ...
    function tambah_data_barang($reqId){
        ...
        $reqSerialPart= $this->input->post('reqSerial');
        $reqCurrencyPart= $this->input->post('reqCurrency');
