...
class issue_po_detail_json extends CI_Controller
{
		...
		while ($issue_po_detail->nextRow()) {
			$row = array();
			  $ids  = $issue_po_detail->getField($aColumns[0]);
			   ...
			   $mata_uang=$issue_po_detail->getField('INISIAL');
	...
	function add()
	{
		...
		$reqBayar2  = $this->input->post('reqBayar2');	
		...
		$issue_po_detail->setField("STATUS_BAYAR", ValToNullDB($reqBayar));
		...
		$reqDiskonPercent = $this->input->post('reqDiskonPercent');
		$reqDiskon = $this->input->post('reqDiskon');
		$reqMaker  = $this->input->post('reqMaker');
			
		$reqTermsAndCondition = $_POST["reqTermsAndCondition"];
		$issuepo = new IssuePo();
		$issuepo->setField('DISKON',$reqDiskon);
		$issuepo->setField('DISKON_PERCENT',ifZero2(dotToNo($reqDiskonPercent)) );
		$issuepo->setField('ISSUE_PO_ID',$reqId);
		$issuepo->setField('STATUS_BAYAR',$reqBayar2);
		$issuepo->setField('MASTER_PROJECT_ID',ValToNullDB($reqMaker));
		$issuepo->updateDiskon();
	...
	function deletedetailNonProject()
	{
		$reqId	= $this->input->get('reqId');
