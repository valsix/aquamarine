...
class aduan_json extends CI_Controller {
	...
	function json()
	{
		...
		$aColumns		= array("ADUAN_ID", "NIP", "NAMA", "ADUAN", "LINK_FILE", "BALASAN");
		$aColumnsAlias	= array("ADUAN_ID", "NIP", "NAMA", "ADUAN", "LINK_FILE", "BALASAN");
		...
		$allRecord = $aduan->getCountByParams(array(), $statement_privacy.$statement);
		// echo $allRecord;exit;
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else
			$allRecordFilter =  $aduan->getCountByParams(array(), $statement_privacy.$statement);

		 $aduan->selectByParams(array(), $dsplyRange, $dsplyStart, $statement_privacy.$statement, $sOrder);
	...
	function add()
	{
		...
		{
			$aduan->setField("UPDATED_BY", $this->USERNAME);
			$aduan->update();
		}
