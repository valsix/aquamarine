...
class issue_po_non_project_detail_json extends CI_Controller
{
	...
	function add()
	{
		...
		$issuepononproject = new IssuePoNonProject();
		$issuepononproject->setField("TANGGAL_BAYAR",dateToDBCheck($reqTanggalPembayaran));
		$issuepononproject->setField("ISSUE_PO_ID",$reqId);
		$issuepononproject->updateTanggalBayar();

		$reqDiskonPercent = $this->input->post('reqDiskonPercent');
		$reqDiskon = $this->input->post('reqDiskon');
		$issuepo = new IssuePoNonProject();
