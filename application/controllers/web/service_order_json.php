...
class service_order_json extends CI_Controller
{
    ...
    function json()
    {
        ...
        $aColumns = array(
            "SO_ID","NO", "NO_ORDER", "PROJECT_NAME", "COMPANY_NAME", "VESSEL_NAME", "VESSEL_CLASS","VESSEL_TYPE", "SURVEYOR", "DESTINATION", "SERVICE", "DATE_OF_START", "DATE_OF_FINISH",
            "EQUIPMENT", "DATE_OF_SERVICE","CONTACT_PERSON"
        );
        $aColumnsAlias = array(
            "SO_ID","NO", "NO_ORDER", "PROJECT_NAME", "COMPANY_NAME", "VESSEL_NAME", "VESSEL_CLASS","VESSEL_TYPE", "SURVEYOR", "DESTINATION", "SERVICE", "DATE_OF_START", "DATE_OF_FINISH",
            "EQUIPMENT", "DATE_OF_SERVICE","CONTACT_PERSON"
        ...
        if(!empty($reqCariTypeVessel)){
            $statement_privacy .= " AND  A.VESSEL_TYPE = '".$reqCariTypeVessel."' ";
        } 
          if(!empty($reqCariProject)){
            $statement_privacy .= " AND  UPPER(A.PROJECT_NAME) LIKE '%".strtoupper($reqCariProject)."%' ";
        } 
        ...
        if(!empty($reqCariPeriodeYear) && !empty($reqBulan) ){
            ...
            //     $statement_privacy .= " AND A.DATE_OF_SERVICE BETWEEN TO_DATE('".$mtgl_awal."','dd-mm-yyyy') AND  TO_DATE('".$mtgl_akhir."','dd-mm-yyyy')";  
                 $statement_privacy .= " AND   TO_DATE(A.DATE_OF_SERVICE,'mmyyyy')='".$reqBulan.$reqCariPeriodeYear."' ";  
             }
        ...
        if(!empty($reqCariGlobal)){
            ...
            $statement_privacy .= " OR UPPER(A.PROJECT_NAME) LIKE '%".strtoupper($reqCariGlobal)."%' ";
        ...
        $statement .= " AND (  UPPER(A.SERVICE) LIKE   '%".strtoupper($_GET['sSearch'])."%' ";
        $statement .= " OR UPPER(A.COMPANY_NAME) LIKE '%".strtoupper($_GET['sSearch'])."%' ";
        $statement .= " OR UPPER(A.VESSEL_NAME) LIKE '%".strtoupper($_GET['sSearch'])."%' ";
         $statement .= " OR UPPER(A.PROJECT_NAME) LIKE '%".strtoupper($_GET['sSearch'])."%' ";
        $statement .= " OR UPPER(A.NO_ORDER) LIKE '%".strtoupper($_GET['sSearch'])."%' ) ";
        // $statement .= " AND (UPPER(NO_ORDER) LIKE '%" . strtoupper($_GET['sSearch']) . "%')";
          $_SESSION[$pg."reqCariSession"] =  $statement_privacy.$statement ;
 ...
 $nomer=0;
    ...
    function add_new(){
        ...
        $reqVesselClass     = $this->input->post('reqVesselClass');
       ...
       // $reqObligation      = $this->input->post('reqObligation');
        ...
        $service_order->setField("COMPANY_ID", ValToNullDB($reqCompanyId));
        $service_order->setField("COMPANY_NAME", $reqCompanyName);
        $service_order->setField("VESSEL_ID", ValToNullDB($reqVesselId));
        $service_order->setField("VESSEL_NAME", $reqVesselName);
        $service_order->setField("VESSEL_TYPE", $reqVesselType);
        $service_order->setField("VESSEL_CLASS", $reqVesselClass);
        ...
        $service_order->setField("DATE_OF_SERVICE", dateToDBCheck($reqDateOfService));
        $service_order->setField("DATE_OWR", dateToDBCheck($reqDateOWR));
        $service_order->setField("PIC_EQUIP", $reqPicEquip);
        $service_order->setField("CONTACT_PERSON", $reqContactPerson);
        $service_order->setField("DOC_LAMPIRAN", $reqDocLampiran);
        $service_order->setField("PATH_LAMPIRAN", $reqPathLampiran);
        $service_order->setField("FINANCE", $reqFinance);
        $service_order->setField("PENANGGUNG_JAWAB_ID", ValToNullDB($reqTTD));
        ...
        $reqSuryeiOwr = $this->input->post('reqSuryeiOwr');
        $this->load->model("ServiceOrder");
          $serviceorder = new ServiceOrder();
         $serviceorder->selectByParamsMonitoring(array("A.SO_ID"=>$reqId));
         $serviceorder->firstRow();
         $reqOfferId = $serviceorder->getField('OFFER_ID');
        $serviceorder = new ServiceOrder();
        $serviceorder->setField("SO_ID", $reqId);
        $serviceorder->setField("OFFER_ID", $reqOfferId);
        $serviceorder->setField("PATH_LAMPIRAN", $str_name_path);
        $serviceorder->setField("OWR_SURYEVOR", $reqSuryeiOwr);
        $serviceorder->updatePath();
        $serviceorder->update_owr_suryevor();
