...
class master_psc_json  extends CI_Controller
{
	...
	function json()
	{
		...
		$dokumen = new MasterPsc();
		// var_dump($dokumen);
		// echo $reqKategori;exit;

		$aColumns		= array("MASTER_PSC_ID","NAMA","KETERANGAN","AKSI");
		$aColumnsAlias	= array("MASTER_PSC_ID","NAMA","KETERANGAN","AKSI");
		...
		if (isset($_GET['iSortCol_0'])) {
			...
			if (trim($sOrder) == "ORDER BY A.MASTER_PSC_ID asc") {
				/*
				...
				*/
				$sOrder = " ORDER BY A.MASTER_PSC_ID desc";
			}
		}
	...
	function delete(){
		...
		$bank = new MasterPsc();
		$bank->setField('MASTER_PSC_ID',$reqId);
		$bank->delete();
