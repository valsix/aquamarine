				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					$.each(files, function (i, file) {
						var v = String(file.name || '' ),
								...
								label =	$(
										(
											'<table><tr><td><span class="MultiFile-label" title="' + t + '">'+
												'<span class="MultiFile-title"></span>'+
												(MultiFile.preview || $(slave).is('.with-preview') ? p : '' )+
											'</span></td><td>&nbsp;&nbsp;&nbsp;</td><td><input type="text" name="reqJudulFile[]" class="easyui-validatebox textbox form-control" style="width:400px"></td></tr></table>'
										)
