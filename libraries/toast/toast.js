...
(function ($) {
    ...
    function render(opts) {
        ...
        if (dismissible) {
            html += `<button type="button" class="ml-2 mb-1 close" data-dismiss="toast" onClick="del_toast('${id}')" aria-label="Close">
  ...
  // $('#'+id).remove();
        if (!$.toastDefaults.stackable) {
            ...
            toastContainer.find('.toast:last').show();
        } else {
            toastContainer.append(html);
              toastContainer.find('.toast:last').show();
        }
